...
//                      <button style={{border:'2px solid black' }} onClick={() => navigate(-1)} className="text-[#f7a221] font-bold mb-4 hover:underline">Go back</button>
...
const WishlistPage = () => {
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-8 relative">
        {/* Back button for empty wishlist */}
        <button 
          onClick={() => navigate(-1)} 
          className="absolute top-6 left-6 flex items-center gap-2 bg-white border-2 border-[#f7a221] text-[#f7a221] font-bold px-4 py-2 rounded-lg hover:bg-[#f7a221] hover:text-white transition-all duration-300 shadow-md z-10"
        >
          <span>←</span> Go back
        </button>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 relative">
      {/* Back button for main wishlist view */}
      <button 
        onClick={() => navigate(-1)} 
          ...
          {items.map((p) => {
            ...
            return (
              ...
              >
                {/* Remove from wishlist button */}
                <div
                  className="absolute top-3 right-3 z-20 bg-black text-white px-3 py-1.5 rounded-full text-xs font-bold hover:bg-red-600 shadow-lg cursor-pointer transition-all duration-300 flex items-center gap-1"
                  onClick={() => remove(p.slug)}
                  title="Remove from wishlist"
                >
                  <Heart size={14} className="fill-red-500 text-red-500" />
