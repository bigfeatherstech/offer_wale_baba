...
const WishlistPage = () => {
  ...
  useEffect(() => {
    console.log('API URL:', import.meta.env.VITE_API_URL || 'No base URL set');
  }, []);

  const handleRemove = async (slug) => {
    try {
      await dispatch(removeFromWishlist(slug)).unwrap();
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#f7a221]"></div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-8">
        <div className="text-center max-w-md">
          <Heart size={64} className="text-red-500 mb-6 mx-auto" />
          <h2 className="text-2xl font-semibold text-gray-900 mb-4">Oops! Something went wrong</h2>
          <p className="text-gray-600 mb-6">{error}</p>
          <div className="flex gap-4 justify-center">
            <button
              onClick={() => dispatch(fetchWishlist())}
              className="bg-[#f7a221] text-white px-6 py-2 rounded-lg font-semibold hover:bg-[#e6941e]"
            >
              Try Again
            </button>
            <button
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-center text-gray-600 mb-8">
          You have <span className="font-bold text-[#f7a221]">{itemCount}</span> item{itemCount !== 1 ? 's' : ''} saved
        </p>
...
// const WishlistPage = () => {
//   useEffect(() => {
//     console.log('',import.meta.env.VITE_API_URL || 'No base URL set');
//   }, []);
